...
async function startServer() {
  ...
  log(
    `Family Print Lab ${app.getVersion()} ready on ${origin}, data in ${DATA}`,
  );
}
        ...
        { label: "Check for updates", click: () => checkForUpdates(true) },
        {
          ...
          click: () =>
            void shell.openExternal(
              "https://github.com/benfavre/family-print-lab",
            ),
        },
...
function checkForUpdates(manual = false) {
  ...
  if (!updater) {
    ...
    updater.on("update-not-available", () => {
      if (manualCheck)
        void dialog.showMessageBox({ message: "You have the latest version." });
    });
    updater.on("error", (e) => {
...
process.on("uncaughtException", (error) => {
  ...
  dialog.showErrorBox(
    "Family Print Lab hit a problem",
    "It will restart. Your projects are safe.",
  );
  app.relaunch();
  app.exit(1);
});
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    dialog.showErrorBox(
      "Family Print Lab could not start",
