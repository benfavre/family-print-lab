...
function freePort(preferred) {
	return new Promise((resolve) => {
		const tryPort = (port) => {
			...
			probe.once('error', () => (port === 0 ? resolve(0) : tryPort(port < preferred + 10 ? port + 1 : 0)));
...
async function startServer() {
	...
	log(`server ready on ${origin}, data in ${DATA}`);
}
	...
	const template = [
		...
		{ role: 'viewMenu' },
		{
			...
			submenu: [
				...
				{ label: 'Project page', click: () => void shell.openExternal('https://github.com/benfavre/family-print-lab') },
...
function checkForUpdates(manual = false) {
	if (!app.isPackaged) {
		if (manual) void dialog.showMessageBox({ message: 'Updates are checked in the installed app only.' });
		return;
	}
	if (!updater) {
		...
		updater.on('error', (e) => {
			...
			if (manualCheck) void dialog.showMessageBox({ type: 'warning', message: 'Could not check for updates right now.' });
		});
	}
	manualCheck = manual;
...
app.on('will-quit', () => process.emit('SIGTERM'));
