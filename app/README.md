...
Plan, design, print and track the family's 3D-printing projects: ideas per person, a model workbench (parametric OpenSCAD parts with live sliders and AI editing, mesh tools, Blender round-trips, full version history), print jobs with slicer settings, a filament shelf that deducts what each print uses, a live read-only link to a Bambu Lab printer, and an AI lab assistant that uses your Claude or ChatGPT subscription. Runs on this computer; data stays in a local SQLite database.
...
On first start the database is created and migrated. If `data/legacy/family.json` (the previous app's data file) exists and the database is empty, it is imported automatically. Existing data can also be restored from the app (⋯ → Import backup).
...
- **Printer** `src/lib/server/printer/`: minimal MQTT client, Bambu report parser, automation (a running print links to the one unlinked Printing job; the job closes itself as Succeeded/Failed), and the simulator.
...
Unit tests use in-memory databases; end-to-end tests use `.e2e/`. Neither touches `data/`. No test calls a real AI provider or printer (the Blender test runs only when Blender is installed): the X2D's exact report fields and the TLS connection to real hardware are still to be verified when the printer is on the network.
